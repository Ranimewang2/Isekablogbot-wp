...
export default async function handler(req, res) {
  ...
  const DEADLINE  = 50_000; // bail at 50s so Vercel's 60s limit never hits

  const elapsed   = () => `${((Date.now() - startTime) / 1000).toFixed(1)}s`;
  const overLimit = () => (Date.now() - startTime) > DEADLINE;
  ...
  for (let i = 0; i < plans.length; i++) {
    ...
    if (overLimit()) {
      console.warn(`[Bot] 50s deadline reached before article ${i+1} — returning early`);
      runResults.errors.push({ step: 'deadline', error: 'Time limit reached — reduce articlesPerRun in config.js' });
      break;
    }
