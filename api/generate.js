...
export default async function handler(req, res) {
  ...
  try {
    ...
    const rawPlans       = planArticles(topics, settings.siteUrl, articlesPerRun + 3); // extra buffer for dedup
    ...
    for (const plan of rawPlans) {
      ...
      if (alreadyDone) {
        console.log(`[Step 3] Skip (published): ${slug}`);
        continue;
      }
      if (blacklisted) {
        console.log(`[Step 3] Skip (blacklisted): ${slug}`);
        continue;
      }
    ...
    if (plans.length === 0) {
      const msg = 'No new topics available — all candidates already published';
      console.warn('[Step 3]', msg);
      await logRun(db, { ...runResults, error: msg, success: true, durationMs: Date.now() - startTime });
      return res.status(200).json({ ok: true, message: msg, ...runResults });
    }
