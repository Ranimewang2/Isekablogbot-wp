...
async function callOpenRouter(apiKey, model, messages, maxTokens = 3000) {
...
async function callWithFallback(apiKey, messages, maxTokens = 3000) {
  for (const model of MODELS) {
    try {
      console.log(`[Writer] Trying model: ${model}`);
      const result = await callOpenRouter(apiKey, model, messages, maxTokens);
      console.log(`[Writer] Success with: ${model}`);
      return { result, model };
    } catch (e) {
      console.warn(`[Writer] Model ${model} failed: ${e.message}`);
    }
  }
